...
URL = 'https://auto.ria.com/newauto/search/?page=1&markaId=31'
...
def save_file(items, path):
    with open(path, 'w', newline='') as file:
        ...
        writer.writerow(['Марка', 'Ссылка', 'Цена в доллорах', 'Цена в гривнях', 'Город'])
        for item in items:
            writer.writerow([item['title'], item['link'], item['usd_price'], item['uah_price'], item['city']])

def parse():
    URL = input('Take your URL in site auto.rio: ')
    URL = URL.strip()
    html = get_html(URL)
